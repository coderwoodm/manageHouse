<template>
  ...
  >
    ...
    <a-form-model-item label="合租性别" prop="sex">
...
export default {
  data:()=>({
      ...
      rules: {
        ...
        rentOut: [{ required: true, message: '请输入出租情况!',trigger:"onsubmit"}],
        sex:[{ required: true, message: '请输入合租性别!',trigger:"onsubmit"}]
      },
  }),
  methods:{
    onSubmit() {
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            let obj = this.form;
            this.$http.post("/AddItemMesTwo",obj).then(res=>{
              console.log(res);
            })
            alert('添加成功!');

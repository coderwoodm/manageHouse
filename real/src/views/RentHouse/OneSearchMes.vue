<template>
  ...
  >
    ...
    <a-form-model-item ref="monthMoney" label="每月价格" prop="monthMoney">
      <a-input
...
<script>
// import ostable from "./Table/OneSearchTable.vue";
export default {
  created(){
  ...
  methods: {
    onSubmit() {
        let obj = this.form;
        if (obj.houseArea == "" && obj.monthMoney == "" && obj.houseType == "" && obj.houseStyle == "" && obj.finishCase == "") {
          this.resetForm();
        }else{
          this.$http.post("/getScreenMes",obj).then(res=>{ 
            this.searchList = res.data;
          });
        }
    },
